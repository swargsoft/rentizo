...
const OWNER_TABS = [
  ...
  { label: 'Branches',  icon: <StorefrontIcon />, path: '/owner/branches' },
...
export default function AppLayout({ children, title, showBack = false }) {
  ...
  const currentTab = useMemo(() => {
    const idx = tabs.findIndex(t => location.pathname.startsWith(t.path) && t.path !== '/owner' && t.path !== '/rider')
    if (idx !== -1) return idx
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100dvh', bgcolor: 'background.default' }}>
      ...
      <AppBar position="sticky" elevation={0}>
        <Toolbar sx={{ gap: 1, minHeight: '56px !important' }}>
          {showBack && (
            <IconButton edge="start" onClick={() => navigate(-1)} sx={{ color: 'text.primary', mr: 0.5 }}>
              <ArrowBackIcon />
            </IconButton>
          )}
      ...
      {/* Bottom Navigation */}
      <BottomNavigation value={currentTab} onChange={handleTabChange} sx={{ flexShrink: 0 }}>
        {tabs.map((tab, i) => (
          <BottomNavigationAction
            ...
            icon={
              tab.label === 'Cart'
                ? <Badge badgeContent={cartItems} color="primary" max={9}><ShoppingCartIcon /></Badge>
                : tab.icon
            }
          />
        ))}
