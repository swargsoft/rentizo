...
export default function Login() {
  ...
  async function doLogin(profile) {
    ...
    try {
      ...
      navigate(
        isNewUser ? '/role' : (currentRole === 'owner' ? '/owner' : '/rider/discover'),
        { replace: true }
      )
    } catch (err) {
    ...
    <Box sx={{
      ...
      <Box sx={{
        ...
        <Stack direction="row" flexWrap="wrap" gap={1} justifyContent="center" sx={{ mt: 2 }}>
          {['No middleman', 'Own your data', 'UPI payments', 'Works offline'].map(tag => (
            <Box key={tag} sx={{
              px: 1.5, py: 0.5,
              borderRadius: 10,
      ...
      <Stack spacing={2} sx={{ width: '100%', maxWidth: 360 }}>
        ...
        <Button
          ...
          size="large"
          fullWidth
          startIcon={loading ? <CircularProgress size={20} color="inherit" /> : (welcomeBack ? null : <GoogleIcon />)}
        ...
        >
          {loading ? 'Signing in…' : (welcomeBack ? `Continue with ${welcomeBack.name?.split(' ')[0] || 'Google'}` : 'Continue with Google')}
        </Button>
        ...
        {!welcomeBack && (
          <Typography variant="caption" sx={{ color: 'text.secondary', textAlign: 'center', lineHeight: 1.6 }}>
            ...
            <a href="/privacy" target="_blank" style={{ color: '#fff', textDecoration: 'underline' }}>
              Privacy Policy
            </a>.
          </Typography>
        )}
