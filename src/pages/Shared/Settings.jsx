...
import { Box, Typography, Stack, Button, TextField, Switch, FormControlLabel, Divider, Alert, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Dialog, DialogTitle, DialogContent, DialogActions, Link, Chip, CircularProgress } from '@mui/material'
...
const getRelayDisplay = (relay) => {
  ...
  if (!health) return { label: 'Probing…', color: 'warning.main', bg: 'rgba(255,193,7,0.15)' }
  if (!health.ok) return { label: 'Offline', color: 'error.main', bg: 'rgba(244,67,54,0.15)' }
  if (health.latencyMs > 2000) return { label: `${health.latencyMs}ms`, color: 'warning.main', bg: 'rgba(255,193,7,0.15)' }
      ...
      <Box sx={{ p: 2, pb: 4 }}>
        <Stack spacing={3}>
          ...
          <Box>
            ...
            <Box sx={{ p: 1.5, bgcolor: '#141414', borderRadius: 1, border: '1px solid #2A2A2A', mb: 1.5 }}>
          ...
          <Box>
            <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1.5 }}>
              ...
              <Stack direction="row" spacing={1}>
                <Button 
                  startIcon={probing ? <CircularProgress size={16} /> : <RefreshIcon />} 
                  variant="outlined" 
                  size="small" 
            ...
            <TableContainer sx={{ borderRadius: 1, border: '1px solid #2A2A2A', bgcolor: '#141414' }}>
              <Table size="small">
                <TableHead>
                  <TableRow sx={{ bgcolor: '#1E1E1E' }}>
                    <TableCell sx={{ color: 'text.secondary', fontWeight: 600, fontSize: '0.75rem' }}>Relay URL</TableCell>
                    <TableCell align="right" sx={{ color: 'text.secondary', fontWeight: 600, fontSize: '0.75rem' }}>Status</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {currentRelays.map(relay => {
                    ...
                    return (
                      <TableRow key={relay} sx={{ '&:hover': { bgcolor: '#1A1A1A' } }}>
                        <TableCell sx={{ color: 'text.primary', fontSize: '0.75rem', fontFamily: 'monospace', wordBreak: 'break-all' }}>
                          {relay}
                        </TableCell>
          ...
          <Stack spacing={2}>
            ...
            <Box sx={{ p: 1.5, bgcolor: '#1E1E1E', borderRadius: 1, border: '1px solid #2A2A2A' }}>
              ...
              <Link
                ...
                sx={{ display: 'flex', alignItems: 'center', gap: 0.5, cursor: 'pointer', textDecoration: 'none', color: 'primary.main', fontSize: '0.85rem', fontWeight: 600, '&:hover': { textDecoration: 'underline' } }}
              >
