...
import { BikeScooterRounded, FmdGoodRounded } from "@mui/icons-material";
...
export default function OwnerDashboard() {
  ...
  const branches = useLiveQuery(
    () =>
      pubkey ? db.branches.where("ownerPubkey").equals(pubkey).count() : 0,
    [pubkey],
  );

  const listings = useLiveQuery(
    () =>
      pubkey ? db.listings.where("ownerPubkey").equals(pubkey).count() : 0,
    [pubkey],
  );
